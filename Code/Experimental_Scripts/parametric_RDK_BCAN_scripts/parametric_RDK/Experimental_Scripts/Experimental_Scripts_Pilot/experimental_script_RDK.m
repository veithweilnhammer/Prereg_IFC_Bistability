...
clear all
close all
root_dir = 'C:\PROJEKTE\Veith\Kathis Exp\RDK\';
...
%% run Settings
SettingsName= input('Please enter name of Settings_File: ', 's');
...
[w, rect] = Screen('OpenWindow', screenNumber, 0,[1280 0 2*1280 960], 32, doublebuffer+1, [], 128); % adjust antialiasing by last number
...
[B I] = sort(Results.disambiguation);

figure()
subplot(2,1,1)
bar([1:length(I(mod(I,2) == 1))], Results.r_bias(I(mod(I,2) == 1))); ylim([0 1])

subplot(2,1,2)
bar([1:length(I(mod(I,2) == 0))], Results.p_correct(I(mod(I,2) == 0))); ylim([0 1])
