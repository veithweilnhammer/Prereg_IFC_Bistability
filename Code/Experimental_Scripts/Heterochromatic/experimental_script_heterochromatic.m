...
n_trials=10;
red_value=100; %pp_managemat(2,5);
blue_channel=3; %pp_managemat(2,6);
cond_per_trial=round(rand(1,n_trials))+1;
red_val_per_trial=repmat(red_value,1,n_trials);
...
for i=1:n_trials
    ...
    if i>1
        fprintf(' [Duration of last trial was %0.1f seconds, adjusted blue value was %0.2f]\n',toc,final_adjusted_color(i-1,blue_channel));
        tic
    else
        fprintf('\n');
    end
    ref_color=[red_val_per_trial(i) 0 0];
    ...
    random_start_value=min([255 floor(rand(1)*2*red_val_per_trial(i))]); % start with a random value between 0 and twice the red value (as long as it is not greater than 255)
    adjusted_color(blue_channel)=random_start_value;
    ...
    while break_trial==0
        ...
        for f=1:frames_per_color
            ...
            if pressed==1
                if keyCode(keyswanted(3)) &&  adjusted_color(blue_channel)<255 % up
                    adjusted_color(blue_channel)=adjusted_color(blue_channel)+1;
                elseif keyCode(keyswanted(4)) && adjusted_color(blue_channel)>0 % down
                    adjusted_color(blue_channel)=adjusted_color(blue_channel)-1;
...
fprintf('values of the blue channel: M=%0.2f SD=%0.2f Median=%0.2f Min=%0.2f Max=%0.2f\n',mean(final_adjusted_color(:,blue_channel)),std(final_adjusted_color(:,blue_channel)),median(final_adjusted_color(:,blue_channel)),min(final_adjusted_color(:,blue_channel)),max(final_adjusted_color(:,blue_channel)));
