...
%
% Additional parameters are (Pregistration V. Weilnhammer):
%         est.p_prc.InitPi: Initial precision of stability prior
%         est.p_prc.StereoPi: Initial precision of stereo precision
%
...
%
% Additional quantaties are (Pregistration V. Weilnhammer):
%         est.traj.q: Probability of rightward rotation
%         est.traj.daq: Perceptual prediction error
%
...
% Initial precision of the stability prior
c.logInitPiMu = [log(1)];
c.logInitPiSa = [0.5];

% weight on the prediction error (not relevant here)
c.logErrorPiMu = [log(0.1)];
c.logErrorPiSa = [0];

% Precision of the disambiguating signal
c.logStereoPiMu = [log(1)];
c.logStereoPiSa = [0.5];
