...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        %%
        %% Modification to original tapas toolbox: Start
        %%
        %% Perceptual level
        % ~~~~~~~~~
        % Numerical representation of perceptual outcomes
        percept2=1;  percept1=0;
        
        
        %% Perceptual Stability
        Start_of_Block = find([0; diff(u(:,4))] > 2);
        
        if any(k==Start_of_Block) % No prediction at the start of a block (i.e., immediately after fixation
            Prediction(k,1)=0.5;
            Precision(k,1)=0;
            
        else
            Prediction(k,1)=u(k-1,1); % Mean of stability prior given by the previous perceptual decision
            PerceptualAge(k,1)=k-max(find(NewPerceptualDecision(1:k-1,1)==1));
            
            if InitPi~=0
                if PerceptualAge(k,1)==1
                    Precision(k,1)=InitPi; % Free Parameter. Reset of the precision of the stability prior after a new perceptual decision
                else
                    Precision(k,1)=Precision(k-1,1)*exp(-abs(daq(k-1))); % Reduction of the precision of the stability prior by the prediction error
                end
            else
                Precision(k,1)=0;
            end
        ...
        %% StereoCue
        Precision(k,2)=StereoPi; % Free Parameter. Precision of the disambiguation
        Prediction(k,2)=u(k,3); % Direction of the disambiguation
        
        
        %% Integration of stereodisparity and stability prior into a "joint prior" of the causes of sensory stimulation
        ...
        q(k,1)= 1/(ratio(k,1)+1); % probability of rightward rotation
        
        
        %% Prediction error
        daq(k,1) = u(k,1) -q(k,1); % prediction error signal (PE)
        if k ~= 2
            w_daq(k,1) = daq(k)*(ErrorPi/Precision(k,1)); % weighted prediction error signal (not relevant for this pre-registration)
        else
            w_daq(k,1) = 0;
        end
        
        %% Checking for new perceptual transitions
        if any(k==Start_of_Block), NewPerceptualDecision(k,1)=1;
        else
        ...
        %%
        %% Modification to original tapas toolbox: End
        %%
        %%
        %% 2nd level
        % ~~~~~~~~~
