...
% = spm_input('Slice time correction', 1, 'm', {
%     'Do not perform STC'
%     'STC before realignment'
%     'STC after realignment'
%     }, [0 1 2], 1);

nSess = length(run_folders); %spm_input('Number of sessions', '+1', 'n', '1', 1);
...
switch STC
    ...
    case 1
        ...
        for s = 1:nSess % session s
            clear files
            files = dir(fullfile(parent_folder, 'functional',experiments_to_preprocess{exp}, run_folders(s).name, 'f*.nii'))
           
            for f=1:length(files)
            matlabbatch{iST}.spm.temporal.st.scans{1, s}(f,1) = {fullfile(parent_folder, 'functional',experiments_to_preprocess{exp}, run_folders(s).name, files(f).name)};
            end
        ...
        matlabbatch{iST}.spm.temporal.st.nslices = 32; %% double check
        matlabbatch{iST}.spm.temporal.st.tr = 2;
        matlabbatch{iST}.spm.temporal.st.ta = 2 - ( 2 / 32 );
        matlabbatch{iST}.spm.temporal.st.so = [32:-1:1];
        matlabbatch{iST}.spm.temporal.st.refslice = round(32/2);
        matlabbatch{iST}.spm.temporal.st.prefix = 'a'
...
ngaus  = [1 1 2 3 4 2];
native = [1 1 1 0 0 0];
anatomy_file = dir(fullfile(parent_folder, 'anatomy','s*.nii'));
matlabbatch{iSeg}.spm.spatial.preproc.channel.vols = {fullfile(parent_folder, 'anatomy', anatomy_file.name)} ;
for c = 1:6 % tissue class c
    matlabbatch{iSeg}.spm.spatial.preproc.tissue(c).tpm = {fullfile(spm('dir'), 'tpm', sprintf('TPM.nii,%d', c))};
...
matlabbatch{iSeg}.spm.spatial.preproc.warp.write = [1 1];
